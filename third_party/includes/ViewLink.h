...
/*-------------------------------    一、常量定义  ------------------------------------------*/
#define VLK_YAW_MAX 180.0               /** the max value of yaw */
#define VLK_YAW_MIN -180.0              /** the min value of yaw */
#define VLK_PITCH_MAX 90.0              /** the max value of pitch */
#define VLK_PITCH_MIN -90.0             /** the min value of pitch */
#define VLK_MAX_YAW_SPEED 2000.0        /** the yaw max moving speed (20 degrees per second)*/
#define VLK_MAX_PITCH_SPEED 2000.0      /** the pitch max moving speed (20 degrees per second)*/
#define VLK_MIN_ZOOM_SPEED 1            /** the min zoom (or focus) speed*/
#define VLK_MAX_ZOOM_SPEED 8            /** the max zoom (or focus) speed*/



/*-------------------------------    二、结构体定义  ------------------------------------------*/

// device connection type   支持连接类型：串口、TCP
 ...
 *  @param pConnParam connect information, including connection type (TCP or serial port),
 *                    ip address, port, serial port name, baudrate
 *  @param pConnStatusCB  connnection status callback function, it will be called by SDK 
 *                      when device is connected or disconnected
 *  @param pUserParam user parameters, will be passed back when pConnStatusCB is called
 *  @return VLK_ERROR_NO_ERROR on success, error code otherwise
 *  @details        this function is asynchronous, the connect result will be passed back by 
 *              callback function, do not use return value to judge connect result.
 *                  SDK allows only one connection, it means if you create a new connection, 
 *              the previous one will be disconnected automatically
 */
 ...
 *  @details sSpeed must be a short number 
 *              from VLK_MIN_ZOOM_SPEED to VLK_MAX_ZOOM_SPEED, the bigger the faster
 */
...
/** @brief Stop zoom
 *  @details once you call VLK_ZoomIn or VLK_ZoomOut, 
 *              Camera will keep zooming until it reach the limitation or VLK_StopZoom is called
 */
